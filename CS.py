...
from PyQt5.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QLabel, QTextEdit, QTabWidget, QSpinBox, QGroupBox,
...
def generate_patterns(stock_length, pieces, kerf=5):
    ...
    for length, qty, name in pieces:
        max_fit = 0
        current_length = 0
        while current_length + length <= stock_length:
            max_fit += 1
            current_length += length
            if current_length < stock_length:
                current_length += kerf
        max_counts.append(min(max_fit, qty))
    for counts in product(*[range(m+1) for m in max_counts]):
        if sum(counts) == 0: continue
        total_length = sum(counts[i] * pieces[i][0] for i in range(n))
        num_cuts = sum(counts)
...
class SolverThread(QThread):
    ...
    def __init__(self, stock_data, demand_100, demand_150, kerf):
        super().__init__()
    ...
    def run(self):
        try:
            results = {
                "diameter_100": self.solve_diameter(100, self.demand_100),
                "diameter_150": self.solve_diameter(150, self.demand_150)
            }
...
class CuttingStockApp(QWidget):
    def __init__(self):
        ...
        self.num_stocks_spin.setRange(1, 10) # <--- LIMITE 10
    ...
    def update_demand_table_150(self):
        ...
        labels = [f"Type {i+5}" for i in range(num_types)]
    ...
    def start_solver(self):
        try:
            # GESTION CONTRAINTE DE NON-NÉGATIVITÉ et Validation
            
            # 1. Validation STOCK
            stock_data = []
            num_stocks = self.stock_table.rowCount()
            for i in range(num_stocks):
                length = float(self.stock_table.item(i, 0).text())
                cost = float(self.stock_table.item(i, 1).text())
                avail = float(self.stock_table.item(i, 2).text())
            ...
            if kerf < 0: raise ValueError("Le Kerf ne peut pas être négatif")
            
            # 3. Validation DEMANDES Ø100
            demand_100 = []
            num_types_100 = self.demand_table_100.rowCount()
            for i in range(num_types_100):
                length = float(self.demand_table_100.item(i, 0).text())
                qty = int(self.demand_table_100.item(i, 1).text())
                
                if length <= 0: raise ValueError(f"Ø100mm Ligne {i+1} : Longueur doit être > 0")
                if qty < 0: raise ValueError(f"Ø100mm Ligne {i+1} : Quantité ne peut pas être négative")
                
                demand_100.append((length, qty, f"T{i+1}"))
            
            # 4. Validation DEMANDES Ø150
            demand_150 = []
            num_types_150 = self.demand_table_150.rowCount()
            for i in range(num_types_150):
                length = float(self.demand_table_150.item(i, 0).text())
                qty = int(self.demand_table_150.item(i, 1).text())
                
                if length <= 0: raise ValueError(f"Ø150mm Ligne {i+1} : Longueur doit être > 0")
                if qty < 0: raise ValueError(f"Ø150mm Ligne {i+1} : Quantité ne peut pas être négative")
                
                demand_150.append((length, qty, f"T{i+1+num_types_100}"))
            
            # Lancement du calcul
            self.thread = SolverThread(stock_data, demand_100, demand_150, kerf)
            ...
            self.result_text.setText(f"⛔ ERREUR DE SAISIE :\n{str(ve)}")
    ...
    def show_results(self, results):
        ...
        for diameter in [100, 150]:
            ...
            if res.get("infeasible"):
                ...
                continue
            if res['cost'] == 0:
                text += "ℹ️  Aucune demande pour ce diamètre\n\n"
                continue
        ...
        for ax, diameter in zip([ax1, ax2], [100, 150]):
            key = f"diameter_{diameter}"
            res = results[key]
            if not res.get("infeasible") and res['cost'] > 0:
